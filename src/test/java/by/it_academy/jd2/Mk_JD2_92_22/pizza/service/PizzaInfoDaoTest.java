...
public class PizzaInfoDaoTest {
    @Test
    public void pizzaInfoDaoTest() {
        //get access to menuDao layer
        IMenuDao menuDao = MenuDaoSingleton.getInstance();
        //get object menu
        IMenu myMenu = menuDao.read(2);
        System.out.println(myMenu.getName());
        //get list menuRow what correspond this menu
        List<IMenuRow> menuRow = myMenu.getItems();
        for (IMenuRow mR: menuRow){
            mR.toString();
          System.out.println(mR.getInfo().getName());
        }
    }
}
